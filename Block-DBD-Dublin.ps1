...
function Create-FirewallRule {
    ...
    Write-ColorOutput Yellow "`n[3/4] Creating firewall rule..."

    if (-not (Test-Path $DBD_EXE_PATH)) {
        Write-ColorOutput Red "WARNING: Dead by Daylight executable not found at: $DBD_EXE_PATH"
        Write-ColorOutput Yellow "Continuing with rule creation anyway..."
    }
